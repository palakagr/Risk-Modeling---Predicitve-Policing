...
```{r risk_factors, message = FALSE, warning=FALSE, echo = FALSE, results = FALSE}
...
```{r crime_other, message = FALSE, warning=FALSE, echo = FALSE }
drug <- 
  read.socrata("https://data.sfgov.org/resource/tmnf-yvry.json") %>% 
...
```{r regression, message = FALSE, warning=FALSE, echo = FALSE, results = FALSE}
...
```{r vis, message = FALSE, warning=FALSE, echo = FALSE}
## Predicted assualts
reg.summary %>%
...
```{r census, message = FALSE, warning=FALSE, echo = FALSE, results= FALSE}
...
reg.summary %>% 
  ...
  kable(caption = "Mean Error by neighborhood racial context1") %>%
  kable_styling("striped", full_width = F) %>%
  footnote(general_title = "\n",
           general = "Table 3.1")
```
